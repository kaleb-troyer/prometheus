...
class Heuristic(object):
    ...
    def attemptToAim(self, model, flux, a, h, obj_val):
        ...
        flux_ubs_array = numpy.array([value for key, value in model.flux_ubs._data.items()]) # mod: 5/29/2025 fixed comparison error between flux_ubs (not a np array) and other np arrays
        if any(flux+added_flux > flux_ubs_array):
    ...
    def createDataFrame(self,model,helio_list,aim_list,aimpoint_select):
        ...
        '''
        num_zeross = []
        # a_cent = math.ceil(len(aim_list) / 2) # mod: returning an incorrect center a_cent because aim_list is getting modified now
        a_cent = math.ceil(aim_list[-1] / 2)   # mod: assumes that the end of the list always includes last aimpoint
        for h in range(len(helio_list)):
        ...
        # top_edge = df.loc[df['aimpoints']==float(len(aim_list))] # mod: doesn't work because aim_list is partial
        top_edge = df.loc[df['aimpoints']==float(aim_list[-1])]   # mod: I think they wanted to use the last element's value
        bottom_edge = df.loc[df['aimpoints']==1.0]
        # middle = df.loc[df['aimpoints']==float(math.ceil(len(aim_list) / 2))] # mod: doesn't work because dealing with partial aim_list
        middle = df.loc[df['aimpoints']==float(math.ceil(aim_list[-1] / 2))]    # mod: i think they just want the middle aiming point
        edges = pd.concat([top_edge,bottom_edge])
        if len(aim_list) >= 4:
            # mid_up = df.loc[df['aimpoints']==float(1+math.ceil(len(aim_list) / 2))] # mod: now aimlist is partial
            mid_up = df.loc[df['aimpoints']==float(1+math.ceil(aim_list[-1] / 2))] # mod: now aimlist is partial
...
class FluxStoreSize(Heuristic):
    ...
    def getUpdatedAimlist(self,model,a_cent):
        ...
        if len(aim_list) < num_sxns:
            ...
            cycle_count = 0
            incr=0  # location  relative to center where we want to pop element 
            flip=1  # flips between larger than center and smaller than center
            ...
            ### this is old way of doing this - always resulted in remove errors 5.29.25
            # y = 2
            # count = 1
            ...
            # ### remove some aimpoints until below a manageable number

            # while len(aim_list) > num_sxns: # runs while loop until enough elements have been removed
            #     if count % 2 != 0: # only enter if odd number
            #         aim_list.remove(a_cent-y)
            #         print(aim_list,a_cent-y)
            #     else:
            #         aim_list.remove(a_cent+y)
            #         print(aim_list, a_cent+y)
            #         y += 2
