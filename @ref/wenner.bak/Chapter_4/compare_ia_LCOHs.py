...
use this if specifically doing IA without uniform
"""
## import stuff
import sys
...
if __name__ == '__main__':
    ...
    for AR in ARs:
        ...
        # Replacements_UNI.append(report_dict_UNI['N_replaced'])
        ...
        Replacements_IA.append(report_dict_IA['N_replaced'])
    ...
    # ax2=ax.twinx()
    # ax2.set_ylim(bottom=ymin_opex, top=ymax_opex)
    lb2 =ax.scatter(ARs, OPEXs_IA, label='OPEX', color='k', edgecolors='k', marker='D')
    ax.set_xlabel('aspect ratio (H/W)', fontsize=fontsize)
    ax.set_ylabel('cost  (M€)', fontsize=fontsize)
    # ax2.set_ylabel('OPEX (M€)', fontsize=fontsize)
    ax.legend(fontsize=fontsize)
    ...
    plt.savefig(f'imgs/panel_replacements_ia_{Qdes}.png', dpi=300)
    plt.show()
    plt.close()
    ...
    #
    ax.grid(True, which='both',alpha=0.3)
    plt.savefig(f'imgs/Q_htfs_ia_{Qdes}.png', dpi=300)
    plt.show()
    plt.close()
    ...
    ax.grid(True, which='both',alpha=0.3)
    plt.savefig(f'imgs/eta_des_ia_{Qdes}.png', dpi=300)
    plt.show()
    plt.close()
    ...
    ax.set_xlim(left=0.2, right=3.2)
    ax.grid(True, which='both',alpha=0.3)
    plt.savefig(f'imgs/nhstats_ia_{Qdes}.png', dpi=300)
    plt.show()
    plt.close()
    ...
    ax.set_xlim(left=0.2, right=3.2)
    ax.grid(True, which='both',alpha=0.3)
    plt.savefig(f'imgs/summer_eta_means_ia_{Qdes}.png', dpi=300)
    plt.show()
    plt.close()
