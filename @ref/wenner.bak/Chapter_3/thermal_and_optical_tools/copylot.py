...
class CoPylot:
    ...
    def __init__(self, debug: bool = False):
        cwd = os.path.join(os.getcwd(), 'copylot', 'core')
        is_debugging = debug
        if sys.platform == 'win32' or sys.platform == 'cygwin':
            if is_debugging:
                self.pdll = CDLL("../solarpilotd.dll")
            else:
                py_loc =Path(__file__).resolve()    # this line gets the current path's folder
                self.pdll = CDLL(f"{py_loc}/../solarpilot.dll")
