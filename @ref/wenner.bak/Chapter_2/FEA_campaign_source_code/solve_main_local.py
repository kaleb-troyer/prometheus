...
P3 changes: -removed getAvgT function. unused
            -
...
# load the receiver
directory = "srlife_models/"

# modelName   = '300Tf_220dT_10000R_30substeps_4days_14period_1.0MPa_p5el'
modelName   = 'test_case_script_425Tf_190dT_high_R'
...
# load the receiver
#model = receiver.Receiver.load( ('srlife_models/solved_models/'+material+'_'+modelName+"_solved"+".hdf5") )
model = receiver.Receiver.load((directory+modelName+".hdf5")) #this line imports a given model with x panels, tubes, in y configuration, with z flowpath...
...
deformation_mat_string = 'base' #can use 'base' or 'elastic_model' or 'elastic_creep'
...
####Added this 08/26/2025
times_plt=[0,0.5,3.5,6.5,9.5,12.5,13,14,14.5,17.5,20.5,23.5,26.5,27,28,28.5,31.5,34.5,37.5,40.5,41,42,42.5,45.5,48.5,51.5,54.5,55,56]
for time1 in times_plt:
    dmg.plotElementsPolar(model, damage_model, damage_mat, 0, 0, parameter='vm',timeStep=int(np.where(times[:]==time1)[0]) ,save=True,Ratio=0.75)
#####
...
#
index_el_crown=int( ((nr-1)*nt + (nt/4) - nt)-1 ) # this formula is based on my understanding of the element numbering system. I could be wrong, but checks have confirmed this is usually the location of max stress around solar noon for elastic cases (creep will move max stress point)
