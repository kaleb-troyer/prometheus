...
import sys
import os
# sys.path.append('./HALOS/')
# sys.path.append('./HALOS/code/')
sys.path.append('./wenner/Chapter_3/thermal_and_optical_tools/')
sys.path.append('./wenner/Chapter_3/damage_tool/')
sys.path.append('./wenner/Chapter_4/aiming_informer/')
sys.path.append('./wenner/Chapter_4/heuristic/')
sys.path.append('./wenner/Chapter_4/HALOS_code/')
...
class Prometheus():
    ...
    def __init__(self):
        ...
        self._expand_namespace()

    def copy(self):
        return deepcopy(self)

    def _expand_namespace(self):

        #  NOTE: This doesn't work, appended paths are needed for top-level imports.

        appendme = (
            os.path.join(self.paths.chapter3, 'thermal_and_optical_tools'),
            os.path.join(self.paths.chapter3, 'damage_tool'),
            os.path.join(self.paths.chapter4, 'aiming_informer'),
            os.path.join(self.paths.chapter4, 'heuristic'),
            os.path.join(self.paths.chapter4, 'HALOS_code')
        )

        for path in appendme:
            sys.path.append(path)

    def _declare_data_structures(self):

        self._casename = 'compare_3D_heuristic_140'
        ...
        class Paths():
            ...
            case_ins: str = os.path.join(chapter4, 'case_inputs')
            case_dir: str = os.path.join(case_ins, self._casename)
        ...
        class Files():
            ...
            output:   str = f'{self._casename}_results.json'
            case_csv: str = f'{self._casename}.csv'
            case_jsn: str = f'{self._casename}.json'
            fluxmap:  str = f'{self._casename}_3D_heuristic_dev_fluxmap.json'
            fluxgrid: str = None
            img_flux_prof: str = f'{self._casename}_flux_profile.png'
            img_placement: str = f'{self._casename}_image_placements.json'
        self.files = Files()
        ...
        class Receiver():
            ...
            massflow:     float = None
            material:     str = 'A230'
            damage_util:  Optional[object] = None
            use_sp_field: bool = False

            @dataclass
            class Models():
                light: Optional[object] = None
                heavy: Optional[object] = None
            ...
            class Panels():
                length: float = None
                height: float = None
            panel: Panels = field(default_factory=Panels)
            ...
            class Tube():
                ...
                @dataclass
                class Options():
                    is_adjacent_tubes: bool = False # prevents annoying error when initializing the tube
                options: Options = field(default_factory=Options)
            tube: Tube = field(default_factory=Tube)

            @dataclass
            class LTE(): # Lifetime Estimate
                desired: float = 80
                trigger: float = 240 # this is the trigger to start optimizing according to LTE impact
                cutoff:  float = 15
    ...
    def _receiver_model_setup(self): # 1
        '''
        Sets up each receiver model and generates
        or loads all heliostat flux images.
        '''

        # creating light and heavyweight receiver thermal models
        self.rec.model.light = therm_helpers.setup_LWT_thermal_model(
            filestring = os.path.join(self.paths.receiver, self.files.inputs).replace('.json', ''),
            ntubes_sim = 1
        )

        self.rec.model.heavy = therm_helpers.setup_LWT_thermal_model(
            filestring = os.path.join(self.paths.receiver, self.files.inputs).replace('.json', ''),
            ntubes_sim = 3
        )

        # just a list of files needed by the SP flux model
        self.filenames = inputs.readCaseFile(
            os.path.join(self.paths.case_ins, self.files.case_csv)
        )

        # HALOS implementation of SolarPilot API for generating flux images
        self.hel.fluxmodel = sp_module.SP_Flux( # aka flux_instance
            self.filenames, use_sp_field=self.rec.use_sp_field
        )

        self.halos_resolution = self.rec.model.light.Npanels * 3 # ie. the NxN resolution for solarpilot, flux grid, and thermal model
        self.hel.fluxmodel.receiver_data['pts_per_dim'] = self.halos_resolution

        if os.path.exists(os.path.join(self.paths.case_dir, self.files.case_jsn)):
            with open(os.path.join(self.paths.case_dir, self.files.case_jsn), 'r') as file:
                self.hel.fluxmaps = json.load(file) # aka flux_dict
        else: # if case json file does not exist, create the file
            self.hel.fluxmaps = self.hel.fluxmodel.get_single_helio_flux_dict(aim_method='Simple aiming method')
            with open(os.path.join(self.paths.case_dir, self.files.case_jsn), 'w') as file:
                json.dump(self.hel.fluxmaps, file)

        #  NOTE: 'filenames' FILE IS NOT IDIOMATIC, CONSIDER REFACTORING

        self.rec.length = float(self.hel.fluxmodel.receiver_data['length'])
        self.rec.height = float(self.hel.fluxmodel.receiver_data['height'])
        self.rec.area   = self.rec.length * self.rec.height

    def _collecting_image_data(self): # 2
        '''
        Calculates image length, width, and relative
        area by heliostat.

        Function has been refactored to execute in a
        vectorized manner.
        '''

        # To start, each heliostat flux image is organized into a 3D array.
        flux_stack = np.array(
            [self.hel.fluxmaps[key] for key in self.hel.fluxmaps.keys()]
        )

        # Then, maximum column, row, and total values are gathered for each image.
        col_max = flux_stack.max(axis=1) # [key x col] 2D array of all max col vals by key
        row_max = flux_stack.max(axis=2) # [key x row] 2D array of all max row vals by key
        hel_max = flux_stack.max(axis=(1, 2))[:, None] # [key, max(col, row)] 2D array of max vals by key

        # Finally, each image is trimmed and the relative area is calculated.
        k = list(self.hel.fluxmaps.keys())
        l = self.rec.length * (col_max > self.hel.img.cutoff * hel_max).sum(axis=1) / self.halos_resolution
        h = self.rec.height * (row_max > self.hel.img.cutoff * hel_max).sum(axis=1) / self.halos_resolution
        r = (flux_stack > self.hel.img.cutoff * hel_max[:, :, None]).sum(
            axis=(1, 2)
        ) / self.halos_resolution**2

        self.hel.img.data = pd.DataFrame({
            'key': k,
            'length': l,
            'height': h,
            'rel_area': r
        }).set_index('key').sort_values('length', ascending=False)

        # The first ncut rows are discarded (ncut=0 by default).
        self.hel.img.data = self.hel.img.data.iloc[self.hel.ncut:]

    def _calculate_ideal_fluxmap(self): # 3


        self.rec.damage_util = damage_tool.damageTool(self.rec.material)

        # setting up a tube model
        self.rec.tube.model = tube_jwenner.Tube()
        self.rec.tube.model.OD = self.rec.tube.OD
        self.rec.tube.model.twall = self.rec.tube.tw
        self.rec.tube.model.options.is_adjacent_tubes = self.rec.tube.options.is_adjacent_tubes
        self.rec.tube.model.initialize()

        # model selects a mass flow rate based on the design power and outlet temp. range
        #  NOTE: When / where is power and temperature assigned? From inputs file. Should include in des var instead.
        self.rec.massflow = np.sum(self.rec.model.light.operating_conditions.mass_flow)

        self.rec.length = self.rec.model.light.D
        self.rec.panel.length = self.rec.length / self.rec.model.light.Npanels
        self.rec.panel.height = self.rec.model.light.tubes[0][0].length

        # generating ideal fluxmap using damage_util and tube.model (??)
        flowpath_config = f'{self.rec.model.light.npaths}_{self.rec.model.light.start_pt}'
        rec_flux_dict, Q_inc_rec, Areas, A_req = informed_aiming.generate_ideal_fluxmap(
            self.rec.damage_util, self.rec.tube.model, self.rec.lte.desired,
            self.rec.massflow, self.rec.panel.length, self.rec.panel.height,
            flowpath_config
        )

        # i kind of see whats happening here but I don't really understand it.
        # lets perform a series of prints to see if we can make sense of each chunk.

        informed_aiming.save_ideal_fluxmap( # plot and save ideal fluxmap
            rec_flux_dict, os.path.join(self.paths.case_dir, self.files.fluxmap)
        )

        if flowpath_config == '2_crt':
            N_panels_half = len(rec_flux_dict.keys())
            N_panels_ideal = N_panels_half * 2

        ideal_flux_grid = informed_aiming.build_ideal_fluxgrid(
            os.path.join(self.paths.case_dir, self.files.fluxmap), res_y=self.rec.model.light.Npanels,
            H=self.rec.panel.height, W=self.rec.panel.width * N_panels_ideal, flowpath_config=flowpath_config
        )

        # need to check the width and raise error if the ideal fluxmap requires more panels than the receiver has
        if N_panels_ideal != np.round(self.rec.model.light.D / self.rec.panel.length):
            ...
            ideal_flux_grid = informed_aiming.fit_grid_to_receiver(
                ideal_flux_grid, self.rec.model.light, flowpath_config
            )

        informed_aiming.plot_ideal_fluxgrid(
            ideal_flux_grid, self.rec.panel.height, self.rec.model.light.D, flowpath_config, self._casename
        )
        ideal_flux_grid = therm_helpers.increase_flux_resolution_blocked(
            flux_low_res = ideal_flux_grid, ndim_new = self.halos_resolution
        )

        # save the flux grid as a csv if desiring to use in SPT method
        self.files.fluxgrid = f'ideal_fluxgrid_{self.rec.model.light.Qdes}_LTE{self.rec.lte.desired}_for_SPT.csv'
        aiming_df = pd.DataFrame(data=ideal_flux_grid, index=None, columns=None, dtype=None, copy=None)
        aiming_df = aiming_df[::-1]
        aiming_df.to_csv(
            os.path.join(self.paths.aiming, self.files.fluxgrid), header=None, index=None
        )
    ...
    def main(self):
        ...
        self._receiver_model_setup()
        self._collecting_image_data()
...
if __name__ =='__main__': # very important if multiprocessing. Prevents subprocesses from being started
    ...
    case_filename = f"./wenner/Chapter_4/case_inputs/{casename}.csv"
    ...
    if os.path.exists(f"./wenner/Chapter_4/case_inputs/{casename}/{casename}.json"):
        with open (f"./wenner/Chapter_4/case_inputs/{casename}/{casename}.json",) as f:
            flux_dict=json.load(f)
    else:
        flux_dict = flux_instance.get_single_helio_flux_dict(aim_method='Simple aiming method')
        with open(f"./wenner/Chapter_4/case_inputs/{casename}/{casename}.json","w") as f:
    ...
    aiming_df.to_csv(
        os.path.join(os.getcwd(), 'wenner', 'Chapter_4', 'aiming', f'ideal_fluxgrid_{model_lite.Qdes}_LTE{LTE_desired}_for_SPT.csv'),
