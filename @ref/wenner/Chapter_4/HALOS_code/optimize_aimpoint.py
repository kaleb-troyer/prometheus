...
class AimpointOptimizer(object):
    def __init__(self, flux_model, params={"section_id":1,"num_sections":1,
        ...
        if self.solver is None:
            self.solver = "gurobi"
            print('no specified solver, defaulting to', self.solver)
    ...
    def getFluxParameters(self, approx=True):
        """
        calculates flux for each heliostat-aimpoint pairing, at each measurement
        point.
    ...
    def optimize(self, mipgap=0.001, timelimit=600, tee=False, keepfiles=False, warmstart=False):
        """
        ...
        """
        print('enforcing timelimit of',timelimit)
        if warmstart: 
...
if __name__ == "__main__":
    ...
    case_filename = "./../case_inputs/flat_50_ca_case.csv" # addition
    kwargs = inputs.readCaseFile(case_filename) #addition
...
#    r.getNormals()
    ...
    # r.getSurfaceArea()
    # f = field.Field() # old way

    ### make the field
    fparams = {}
    fparams["num_sections"] = 6
    fparams["section_method"] = "angle"
    fparams["mirror_area"] = 100
    fparams["hold_sp_rejects"] = True
    fparams["aim_cols"]=10
    f = field.Field(filenames = kwargs, params = fparams, use_sp_field = True)
    ###
    f.num_sections=fparams["num_sections"]
    # f.GenerateCoords(numpy.array([-200,-100,0,100,200]),numpy.array([-300,-300,-300,-300,-300]),numpy.array([0,0,0,0,0]))
    flux_model = flux_model.FluxModel(None,None,r,flux_method.SimpleNormalFluxCalc(5),weather_filename,f,filenames=kwargs,hour_id=12,use_sp_flux=True) #added filenames variable
