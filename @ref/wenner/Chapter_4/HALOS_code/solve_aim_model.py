...
logging.basicConfig(filename='solve_aim_model-log.log', level=logging.INFO, format='%(asctime)s - %(message)s', datefmt='%d-%b-%y %H:%M:%S')

def get_results_size(results):
    """
    manually sum the total dictionary memory size using for loop
    results - dictionary from solving aimpoint optimization
    """
    mem = 0
    num_sections = len(results)
    for i in range(num_sections):
        for key in results[i].keys():
            mem = mem + sys.getsizeof(results[i][key])
    return mem
...
def subproblemSolve(kwargs):
    ...
    print("Start")
    ao.createFullProblem()
    # logging.info( 'input size is: {:5.0f} bytes'.format(sys.getsizeof(ao)))
    logging.info( 'input pyomo size is: {:5.0f} bytes'.format(asizeof.asizeof(ao.model)))
    print("Problem Created")
    elapsed_build = time.time() - t_start_1 
    print("Time to build: ", elapsed_build)
    ao.optimize(warmstart=True) # can set warmstart=True to try using heuristic
    print("Processing results")
    #d = ao.processResults()
    d = ao.processOutputs()
    # logging.info( 'output size is: {:5.0f} bytes'.format(sys.getsizeof(d)) )
    # logging.info( 'output size is: {:5.0f} bytes'.format(asizeof.asizeof(d.model)) )
    return d
...
def solveDecomposedModel(case_name, case_filename,hour_id = None, 
    ...
    logging.info( 'total result size is: {:5.0f} bytes'.format( get_results_size(results) ) )
...
def runHourlyCase(main_case_name, case_name, case_filename, hour_id = None, decomp = False, parallel = True,
    ...
    """
    # logger.info('running runHourlyCase')
    logging.info('running runHourlyCase')
    if decomp:
