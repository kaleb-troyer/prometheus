...
if __name__ == "__main__":
    ...
    if not load_data:
        ...
        instance=sp_module.SP_Flux(kwargs)
        flux_dict = instance.get_single_helio_flux_dict(aim_method='Image size priority')
    ...
    # Before defocusing
    if True:
        im = plt.imshow(comb_flux,vmin=vmin,vmax=vmax)
        plt.colorbar(im)
        plt.tight_layout()
        plt.savefig('imgs/default_pre_defocus_flux_profile')
        plt.show()
    ...
    if True:
        ...
        plt.colorbar(im)
        plt.tight_layout()
        plt.savefig('imgs/default_post_defocus_flux_profile')
        plt.show()

    W=21.6 #cp.data_get_number(self.r,"receiver.0.rec_width") looks like these are default values right now
    H=12   #cp.data_get_number(self.r,"receiver.0.rec_height")
    Atotal=H*W

    Qpossible=helpers.calc_Qtotal(flux_limit,Atotal)  
    Qresult  =helpers.calc_Qtotal(comb_flux,Atotal)
    Qbefore  =helpers.calc_Qtotal(flux_before_defocus,Atotal)  
    print('the possible power for this is :{:10.2f} kW/m2'.format(Qpossible) )
    print('the delivered power for this is:{:10.2f} kW/m2'.format(Qresult) )
    print('the intended power for this is :{:10.2f} kW/m2'.format(Qbefore) )
    
    im = plt.imshow(flux_limit,vmin=vmin,vmax=vmax)
    plt.colorbar(im)
    plt.tight_layout()
    plt.savefig('imgs/default_flux_limit_profile')
    plt.show()
