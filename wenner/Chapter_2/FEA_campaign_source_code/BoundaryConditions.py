...
def makeTempArray_gflux_wPreheat(r,Tfs,dT_totals, t_ph, t_op, nz, ntheta, times, dayPts, dySteps):
    """
    r - (-) ratio of conduction dT to convection dT
    Tfs - [array] (C) fluid temperature during operation - 
    dT_totals - [array] (C or K) total temperature difference
    Tbase - (C) the ambient base temperature. Assuming 
    t_p - (hr) the time during which the tube isothermally preheats up to operation conditions
    t_op - (hr) the time during which the temperature gradient starts and ends
    nr, nz, ntheta - radial, axial, and circumferencial dimensions of the array
    times - (hr) timestamps of each temperature profile provided to SRLIFE
    period - (hr) total cycle time, which includes preheat, operation, shutdown and offtime
    dayPts - list of integers. Each element tells function how many cycles to repeat the ith Tf and dT_total. Be sure to specify the 1st day specifically and
    dySteps - how many elements in a day
    """
    # innerSurfaceArray=np.array([ [ [] ] ])
    # outerSurfaceArray=np.array([ [ [] ] ])
    for i in range(len(Tfs)):
    #define temperature differences
        dT_cond=dT_totals[i]*(1-1/(1+r)) # temperature difference between outside surface and inside surface
        dT_conv=dT_totals[i]/(1+r)   # temperature difference between inside crown and unheated inside surface
        T_outer_peak=Tfs[i]+dT_totals[i] # crown temperature
        T_inner_peak=T_outer_peak-dT_cond # inner surface crown temperature
        T_outer_back=T_inner_peak-dT_conv # unheated, coldside
        T_base = 30 # (C) base temperature. This is constant and persists even during off hours
    
        #define some operating timepoints
        t_rampP=t_ph # pressure array will use the same ramp up as the temperature array
        t_shutdown=t_op+t_ph # t_shutdown starts once full operation and initial preheat time have elapsed
        
        #make relative temperatures -- the part of each temperature that changes with time (i.e. remove base temp)
        T_f_rel = Tfs[i] - T_base
        # T_outer_peak_rel= T_outer_peak - T_base
        if i==0:
            # create inner wall, bottom: no change in Tf the entire day, surface is isothermal
            fFracArrayInnerBottom=buildFracNormal(times[:int(dySteps+1)],t_ph,t_op)
            fArrayInnerBottom=T_f_rel*fFracArrayInnerBottom
            innerTempArrayBottom=buildSurfUniTemp(fArrayInnerBottom,np.ones(int(ntheta/2)),nz)

            # create inner wall, top: the deltaT_conv will grow throughout the day as flux increases
            fFracArrayInnerDelta=buildDeltaTFracCos_wPH(times[:int(dySteps+1)],t_op,t_ph)
            innerTempArrayTop   =buildSurfCosine(fArrayInnerBottom,makeThetaArray(int(ntheta/2)),fFracArrayInnerDelta,dT_conv,int(ntheta/2),nz)
            ...
            ##
            outerTempArrayTop   = buildSurfCosine(fArrayInnerBottom,makeThetaArray(int(ntheta/2)),fFracArrayInnerDelta,dT_totals[i],int(ntheta/2),nz ) #spatial variance imposed on existing time variance
            outerTempArrayBottom= buildSurfUniTemp(fArrayInnerBottom,np.ones(int(ntheta/2)),nz)

            # add dimensions to the unheated side's array
            outerTempArrayDay1  = np.concatenate( (outerTempArrayTop,outerTempArrayBottom) ,axis=1) 

            # add day 1 array to master array
            innerSurfaceArray=innerTempArrayDay1#np.concatenate( (innerSurfaceArray,innerTempArrayDay1), axis=0 )
            outerSurfaceArray=outerTempArrayDay1#np.concatenate( (outerSurfaceArray,outerTempArrayDay1), axis=0 )
        if i>0: #if not building first day of simulation then indices don't need to include the 0th element
            # create inner wall, bottom: no change in Tf the entire day, surface is isothermal
            fFracArrayInnerBottom=buildFracNormal(times[:int(dySteps+1)],t_ph,t_op) # can use the first day's times because general shape the same
            fArrayInnerBottom=T_f_rel*fFracArrayInnerBottom # this line accounts for the new temperature
            innerTempArrayBottom=buildSurfUniTemp(fArrayInnerBottom,np.ones(int(ntheta/2)),nz) # can use the 
            ...
            # combine top and bottom for total inner wall array day 1
            innerTempArrayDay1  = np.concatenate( (innerTempArrayTop,innerTempArrayBottom) ,axis=1) 
            # make the N days version
            innerTempArrayDayNs=np.tile(innerTempArrayDay1[1:int(dySteps+1)],(dayPts[i],1,1)) #generate array for all days after the first day
            ...
            outerTempArrayDay1  = np.concatenate( (outerTempArrayTop,outerTempArrayBottom) ,axis=1) 
            # make the N days version
            outerTempArrayDayNs=np.tile(outerTempArrayDay1[1:int(dySteps+1)],(dayPts[i],1,1)) #replicate the daily outerTempArray for the number of days to simulate


            # add N days array to master array
            innerSurfaceArray=np.concatenate( (innerSurfaceArray,innerTempArrayDayNs), axis=0 )
            outerSurfaceArray=np.concatenate( (outerSurfaceArray,outerTempArrayDayNs), axis=0 )
        ...
        # outerTempArray=np.concatenate( (outerTempArrayDay1,outerTempArrayDayN), axis=0 ) 
        
        # ## concat the array into master array
        # innerTempArray=np.concatenate( (innerTempArrayDay1,innerTempArrayDayN), axis=0 ) # combine the N days with the 1st day
    
    
    # now that time variance is taken care of, can re-add the time-independent temperature base
    innerSurfaceArray=innerSurfaceArray+T_base
    outerSurfaceArray=outerSurfaceArray+T_base


    return innerSurfaceArray, outerSurfaceArray
