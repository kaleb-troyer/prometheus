...
class WELL512(object):
    # NOTE it seems like scipy.zeros may have been deprecated. I changed all of them to numpy - jwenner
    def __init__(self,state_file,W=32,R=16,P=0,M1=13,M2=9,M3=5,scen=0):
        self.state = np.zeros(R,dtype=np.uint32)    # changed this line on 5/23/25. replaced 'scipy' with 'np' in two places
        self.W = W
        self.R = R
        self.P = P
        self.M1 = M1
    ...
    def readStates(self,f):
        ...
        state_is = []
        import csv
        reader = csv.reader(open(f,'r'))        # google ai says that 'rU' got deprecated in python 3.8
        for line in reader:
            state = []
            for i in range(16):
                state.append(np.uint32(line[i])) # replaced scipy with np
            if len(line) > 16:
                state_i = int(line[-1])
            else: 
                state_i = 0
    ...
    def MAT0POS(self,t,v): 
        return np.uint32(v^(v>>t))
        
    def MAT0NEG(self,t,v): 
        return np.uint32(v^(v<<(-(t))))
        
    def MAT3NEG(self,t,v): 
        return np.uint32(v<<(-(t)))
        
    def MAT4NEG(self,t,b,v): 
        return np.uint32(v ^ ((v<<(-(t))) & b))
        
    def getVariate(self):
        z0 = np.uint32(self.state[self.VRm1()])
        z1 = np.uint32(self.MAT0NEG(-16, self.V0())) ^ np.uint32(self.MAT0NEG(-15, self.state[self.VM1()]))
        z2 = np.uint32(self.MAT0POS(11, self.state[self.VM2()]))
        self.state[self.newV1()] = z1 ^ z2
        self.state[self.newV0()] = self.MAT0NEG(-2, z0) ^ self.MAT0NEG(-18, z1) ^ self.MAT3NEG(-28, z2) ^ self.MAT4NEG(-5, np.uint32(0xda442d24), self.state[self.newV1()])
        self.state_i = (self.state_i + 15) & 0x0000000f
        return self.state[self.state_i] * self.FACT
        
    def getVariates(self,num_samples):
        arr = np.zeros(num_samples,dtype=np.float64)
