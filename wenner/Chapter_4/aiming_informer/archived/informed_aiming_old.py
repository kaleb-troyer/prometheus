...
def generate_ideal_fluxmap(dmg_case, tube, LTE_desired, mflow, W_panel, L_t, flowpath_config):
    """
    ...
    dmg_case.make_contour_function_from_interpolator(LTE_desired, cutoff=270)
...
def generate_ideal_fluxmap_with_offset(dmg_case, tube, LTE_desired, mflow, W_panel, L_t, v_offset, flowpath_config):
    """
    ...
    for (xpt,xpt_key) in zip(xpts,xpt_keys):
        ...
        for j, ypt in enumerate(y_heights_array[:-1]):
            ...
            if flow_config == '2_ctr':
                ...
                print('flow configuation not recognized')
...
def plot_ideal_fluxgrid(fluxgrid, H, W, flow_config,img_name='default_save'):
    ...
    if flow_config == '2_ctr':
        ...
        cb.set_label('incident flux (kW/m2)')
...
def fully_utilize_receiver_maximize_lifetime(dmg_case, tube, mflow, W_panel, W_receiver, L_t, v_offset, flowpath_config):
    """
    ...
    while not is_correct_LTE:
        ...
        if is_offset_zero:
            rec_dict, Q_inc_rec, Areas, A_reqd   =generate_ideal_fluxmap(dmg_case, tube, LTE_des, mflow, W_panel, L_t, flowpath_config)
        # if zero, then implement the old fluxmap 
        else:
            rec_dict, Q_inc_rec, Areas, A_reqd   =generate_ideal_fluxmap_with_offset(dmg_case, tube, LTE_des, mflow, W_panel, L_t, v_offset, flowpath_config)

        # compare the width to the allowable width
        if flowpath_config == '2_ctr':
            nPanels_1side   =len(list(rec_dict.keys()))
            Wreq            =W_panel*len(rec_dict.keys())*2
        else:
            print('flowpath not recognized!')

        # if width is less, increment by (step) years. use a is_fully_utilized boolean
        if (Wreq < W_receiver) and not is_correct_LTE_decade:
            LTE_des =LTE_des+LTE_incr
        # if width is more, decrement by (step/2) years.
        elif Wreq > W_receiver and not is_correct_LTE_decade:
            LTE_des =LTE_des-LTE_incr/2
        elif (Wreq == W_receiver) and not is_correct_LTE_decade:
            print(f'LTE contour decade found: {Wreq/W_panel} panels fit on the {W_receiver} wide receiver')
            print(f'--- LTE selected: {LTE_des} years ---')
        ...
        # if we are in the correct decade, slowly increase the LTE until we add another panel
            ...
            LTE_des =LTE_des+5
        # we were in the correct decade until we added one too many LTE years; decrement
        elif is_correct_LTE_decade and (Wreq != W_receiver):
            LTE_des =LTE_des-5
            is_correct_LTE =True
...
def build_ideal_fluxgrid(filestring, res_y, H, W, flowpath_config):
    ...
    --
    filestring - (str) name and path of ideal flux map json file to load
