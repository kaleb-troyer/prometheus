# Module that implements available damage tools, thermal models, etc.
# to generate a set of aimpoints and advise designs.
# 2025-08-01
# Jacob Wenner
...
def generate_ideal_fluxmap_with_offset(dmg_case, tube, LTE_desired, mflow, W_panel, L_t, v_offset, flowpath_config, show_LTE_ctr=False, cutoff=270):
    ...
    if flowpath_config == '2_ctr':
        ...
        A_req     = np.sum(Areas)*2
    else: print('flowpath not recognized - could not calculate incident power')
...
def plot_ideal_fluxgrid(fluxgrid, H, W, flow_config,img_name='default_save', display=False):
    ...
    display     - (bool) display the figure after saving
    """
