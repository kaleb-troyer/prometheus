# Provides the Parameters class, which cleanly loads parameters as dataclasses
# from config.inputs.py according to the schema outlined in config.schema.py.
# Also provides the Case class, which is responsible for generating an optimized
# solar field layout, ideal flux profile, and individual heliostat flux images
# using the methods and objects created by Jacob Wenner.
# 2026-03-11
# Kaleb Troyer
...
# be called until it is confirmed the parameters can be used to successfully
# generate case products, such as solar field layout. Case products will be
# stored in temporary file storage until each is successful, then they will be
# moved to the hash case directory.

# NOTE: Right now Parameters() is designed only with design-point in mind, but
# a given 'case' (derived from Parameters() and assumed to be design-point) will
# need to be evaluated at off-design conditions. How should fixed field designs
# with a change in time of day (solar angle, DNI, etc) be approached? Either
# many heliostat images can be generated by time of day, or the creative use of
# occluders through core.losses may be used. Is this out-of-scope?

class Parameters():

    # tasks
    # - [x] floating point hash cutoff
    # - [ ] move freeze declarations to Case

    def __init__(self):

        # Design parameters are loaded using `inputs.py` and `schema.py`. The
        # schema ensures only expected parameters of the appropriate data type
        # are passed along to the rest of the program.
        self.sys = {}
        self.rec = {}
        self.hel = {}
        self.plt = {}
    ...
    def load(self):
        """
        Loads parameters from config.inputs.py according to config.schema.py
        established structures. Parameters are loaded under `sys`, `rec`, `hel`,
        `plt`, or `opt`.
        """
    ...
    def update(self, params: dict={}, _obj=None):
        """
        Updates each dataclass with a dictionary modeling the structure of
        schema.py.
        """
        for parkey, parval in params.items():
            ref = _obj if _obj is not None else self._index()[parkey]
            if isinstance(parval, dict):
                self.update(parval, getattr(ref, parkey) if _obj is not None else ref)
            elif hasattr(ref, parkey):
                setattr(ref, parkey, parval)

    def from_json(self, hash):
        """
        Loads parameters from the specified hash according to config.schema.py
        established structures. Parameters are loaded under `sys`, `rec`, `hel`,
        `plt`, or `opt`.
        """
    ...
    def freeze(self):
        """
        Generates a hash value from the relevant input parameters, creates a
        directory named after the hash under cases/, and saves the parameters as
        a json in the directory.
        """
    ...
    def _generate_case_hash(self, *args):
        # Objects are rounded if floating point to prevent rounding errors in
        # long floating point numbers on different systems.
        def _round_floats(obj):
            if is_dataclass(obj):
                return {k: _round_floats(v) for k, v in asdict(obj).items()}
        ...
        parameters = [_round_floats(obj) for obj in args]
    ...
    def _index(self):
        # A private index is used in the .update() method to point key values to
